...
# CLAUDEORCH_NO_KEYCHAIN=1 keeps tests away from the developer's real macOS
# Keychain entry — without it, swap/creds tests overwrite the live login.
test: ## Run unit tests
	CLAUDEORCH_NO_KEYCHAIN=1 go test -race ./...

test-integration: ## Run unit + integration tests
	CLAUDEORCH_NO_KEYCHAIN=1 go test -race ./... && CLAUDEORCH_NO_KEYCHAIN=1 go test -race -tags=integration ./tests/integration/...
